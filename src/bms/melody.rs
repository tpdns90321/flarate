...
impl Melody {
    pub fn apply(&mut self, channel: Channel, value: &str) {
        match channel {
            Channel::Event(event) => match event {
                Event::BGM => check_apply(&mut self.bgm, parse_alphanum(value)),
                Event::Length => check_apply(&mut self.length, parse_float(value)),
                Event::BPM => check_apply(&mut self.bpm, parse_hex(value)),
                Event::ExBPM => check_apply(&mut self.ex_bpm, parse_alphanum(value)),
                Event::Stop => check_apply(&mut self.stop, parse_hex(value)),
            },
            Channel::Object(object) => match object {
                Object::FirstPlayerVisible(k) => {
                    check_apply_object(&mut self.first_player_visible, k, value)
                }
                Object::SecondPlayerVisible(k) => {
                    check_apply_object(&mut self.second_player_visible, k, value)
                }
                Object::FirstPlayerInvisible(k) => {
                    check_apply_object(&mut self.first_player_invisible, k, value)
                }
                Object::SecondPlayerInvisible(k) => {
                    check_apply_object(&mut self.second_player_invisible, k, value)
                }
            },
        };
    }
